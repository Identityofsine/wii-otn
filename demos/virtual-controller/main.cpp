...
void run() {
  ...
  const auto return_value = vigem_connect(client);

  if (!VIGEM_SUCCESS(return_value)) {
    // print error code out in hex
		std::cout << "Failed to connect to driver! Error: " << std::hex << return_value << std::endl;
    return;
  } else {
  ...
  if (!VIGEM_SUCCESS(pad_return_value)) {
	...
	//
	//add a sleep to demonstrate driver connection
	//
	Sleep(2000);
